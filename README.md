...
- **Structured data** — `/about` carries a `WebSite` + `SoftwareApplication` JSON-LD graph. `script-src 'self'` does not block it (ld+json is data, not executable script — verified against the production CSP), and it claims nothing the app has not earned: no ratings, no reviews.
