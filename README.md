...
Alongside it, `npm run test:unit` runs 84 unit tests (no browser, no dependencies): 52 over the pure logic in `logic.js` — streak arithmetic, backup sanitizing, and calm-ledger aggregation — and 32 over the calendar reminder in `reminder.js`. The daylight-saving cases run in child processes with `TZ` set, so they exercise real zones rather than whichever one the machine happens to be in. Both suites run in CI.
