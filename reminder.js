 ...
 * in the journey. First-of-export, halfway and last are fixed; the rest
 * rotate by day so a day keeps its tip across re-exports. */
...
function buildReminderIcs({ track, trackId, lang, time, seq, now, origin, t, completed, schedule }) {
  ...
  for (let k = 0; k < plan.count; k++) {
    ...
    const pageLink = `${origin}${dayPagePath(trackId, track, dayIndex, lang)}`;
    ...
    const description = [
      ...
      "",
      t("reminder.tipLabel", { text: t(tipKeyFor({ position: k, dayIndex, total, trackId })) }),
      "",
    ...
    lines.push(
      ...
      `DESCRIPTION:${icsText(t("reminder.alarmDay", { n, title }))}`,
      "END:VALARM",
      "END:VEVENT"
    );
  }
...
if (typeof module !== "undefined" && module.exports) {
  module.exports = { REMINDER_UID, REMINDER_TIPS, icsText, icsFold, dayPagePath, appDayLink, reminderSchedule, tipKeyFor, weekLabel, buildReminderIcs };
}
