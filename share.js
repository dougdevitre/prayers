...
if (typeof document !== "undefined") (function () {
  ...
  for (var j = 0; j < cards.length; j++) enhanceCard(cards[j]);

  // The day's share card: Download is a plain link and needs nothing. Share
  // image hands the PNG itself to the device's share sheet (Instagram,
  // Messages, WhatsApp as a photo), so it appears only where the browser can
  // share files. The image is fetched on tap, not on load; if that fails,
  // the card opens instead, where a long-press saves it.
  function enhanceCard(figure) {
    var button = figure.querySelector(".share-card-native");
    var link = figure.querySelector(".share-card-image");
    if (!button || !link || !navigator.share || !navigator.canShare || typeof File === "undefined") return;
    var probe;
    try { probe = new File([""], "card.png", { type: "image/png" }); } catch (e) { return; }
    ...
    button.addEventListener("click", function () {
      fetch(link.getAttribute("href"))
        ...
        .then(function (blob) {
          var file = new File([blob], button.getAttribute("data-file") || "stand.png", { type: "image/png" });
          var text = row ? (row.getAttribute("data-text") || "") + "\n" + (row.getAttribute("data-url") || location.href) : location.href;
          return navigator.share({ files: [file], text: text });
        })
        .catch(function (e) {
          // The reader closed the sheet: nothing to do. Anything else: open
          // the card, so it can still be saved and sent by hand.
